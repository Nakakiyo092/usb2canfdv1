...
Each stage in the data path between the host and the CAN bus has its
own buffer.
...
the firmware replaces the buffer with a single `[BELL]` byte so that
the parser responds with `[BELL]` when the eventual `[CR]` arrives.
...
This FIFO has 3 slots of 4 KB each.
...
1. **APP Tx Frame FIFO** is a software circular FIFO into which the
   ...
   that the marker-matching mechanism (described below) can later
...
The firmware matches each Tx event to its APP FIFO entry by marker.
When a match is found, the APP FIFO entry is released and (if Tx
event reporting is enabled) the generator (`Gen.`) produces the
corresponding Tx event report and enqueues it into the CDC Tx
Buffer.
An APP FIFO entry for which no event ever arrives was dropped before
being acknowledged on the bus (for example due to disabled
retransmission, or to an overflow inside the HAL stages) and is
eventually retired without a Tx event report.  Overflow cases
additionally raise the driver-side frame-loss flag (`F` bit 3);
...
- **HAL Tx Event FIFO**: Event entries are read out as the firmware
  ...
  then released without an event when the marker matching detects
  the gap.

Frames still pending in the APP FIFO when the channel is closed are
discarded; the FIFO starts empty on every open.
