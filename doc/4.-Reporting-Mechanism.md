...
Frame reports (both Rx frames and Tx events) are delivered on a best-effort basis. Under heavy host load the CDC Tx buffer may saturate and subsequent reports are silently dropped to avoid blocking firmware execution. The buffer-overflow condition can be inspected via the `F` command.

The reception of CAN frames by the CAN controller and the transmission to the CAN bus are independent of this delivery loss; only the host-side notification is affected.
...
In addition, the device guarantees the following ordering relative to host commands:

- Host transmission commands are processed in the order they are received.
- Tx events are reported in the same order as their corresponding host transmission commands were issued.
