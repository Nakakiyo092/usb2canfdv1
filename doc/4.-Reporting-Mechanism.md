...
This can happen, for example, when the firmware's main loop takes longer than the design window, 
when an interrupt handler consumes excessive time, 
when the nominal bit rate is below 10 kbps, 
or when the data bit rate is below 100 kbps.
