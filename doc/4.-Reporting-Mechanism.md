...
If the timestamp cannot be computed reliably (for example when the firmware's main loop takes longer than the design window), the micro second timestamp is reported as `FFFFFFFF`, a value outside the normal spec range of `00000000` .. `D693A3FF` (0 .. 3,599,999,999 us).
