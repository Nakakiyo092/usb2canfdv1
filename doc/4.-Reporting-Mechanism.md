...
If the timestamp cannot be computed reliably (for example when the firmware's main loop takes longer than the design window), the timestamp is reported as an out-of-spec sentinel:
