...
where `xx` is a hex value with 8 status bits:
  ...
  - bit 7: Sets on any CAN protocol error event (stuff, form, CRC,
    bit, ACK, etc.) in either the arbitration or data phase, and
    on a transition into the bus-off state. Includes events that
    do not increment the error counters (for example a No-ACK
    detected while the device is already error-passive). Routine
    arbitration loss is not a protocol error and does not set this
    bit.
