...
Bit fields of `xx` and `yy` are interpreted the same way as on the SJA1000, then converted to compensate for the clock difference between the SJA1000 (16 MHz) and FDCAN (80 MHz):
...
Sets up CAN nominal bit rate to 83.333 kbps and sampling point to 87.5 %.
...
Sets up CAN FD nominal bit-rate and bit-timing, where `dd`, `xx`, `yy` and `zz` are hex values.
...
Sets up CAN FD data bit-rate, where `n` is 0-2, 4, 5.
...
where `xx` is a hex value with 8 status bits:
  ...
  - bit 7: Sets on any CAN protocol error event.
...
Note:
- The exact format is intentionally left unspecified and may change in future firmware versions. Currently the response is self-documented and human-readable.
...
- The `xx` and `yy` characters may encode the project variant, not just a sequential revision.
