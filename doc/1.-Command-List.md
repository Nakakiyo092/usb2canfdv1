...
```
----------------------------------------------------------------------------------------------------
CMD | BY | STATE | SYNTAX                | DESCRIPTION
----------------------------------------------------------------------------------------------------
'S' | A  |  YES  |  Sn[CR]               | Sets up with CAN FD nominal bit rate where n is 0-8.
    |    |       |                       | S0 10kbps          S3 100kbps         S6 500kbps
    |    |       |                       | S1 20kbps          S4 125kbps         S7 800kbps
    |    |       |                       | S2 50kbps          S5 250kbps         S8 1Mbps
's' | YA |  YES  |  sddxxyyzz[CR]        | Sets up CAN FD nominal bit rate and bit timing,
    |    |       |                       | where dd, xx, yy and zz are hex values.
'Y' | B  |  YES  |  Yn[CR]               | Sets up with CAN FD data bit rate where n is 0-2, 4, 5.
    |    |       |                       | Y0 500kbps         Y3 3Mbps (N/A)     Y6 6Mbps (N/A)
    |    |       |                       | Y1 1Mbps           Y4 4Mbps           Y7 7Mbps (N/A)
    |    |       |                       | Y2 2Mbps           Y5 5Mbps           Y8 8Mbps (N/A)
'y' | YC |  YES  |  yddxxyyzz[CR]        | Sets up CAN FD data bit rate and bit timing,
    |    |       |                       | where dd, xx, yy and zz are hex values.
'O' | A  |  YES  |  O[CR]                | Opens the CAN FD channel in normal mode.
    |    |       |                       | Both sending & receiving are enabled.
'L' | A  |  YES  |  L[CR]                | Opens the CAN FD channel in listen only mode.
    |    |       |                       | Only receiving is enabled.
'C' | A  |  YES  |  C[CR]                | Closes the CAN FD channel.
'r' | A  |  YES  |  riiil[CR]            | Transmits a base remote frame.
'R' | A  |  YES  |  Riiiiiiiil[CR]       | Transmits an extended remote frame.
't' | A  |  YES  |  tiiildd...[CR]       | Transmits a classical base data frame.
'T' | A  |  YES  |  Tiiiiiiiildd...[CR]  | Transmits a classical extended data frame.
'd' | B  |  YES  |  diiildd...[CR]       | Transmits a FD base data frame without bit rate switch.
'D' | B  |  YES  |  Diiiiiiiildd...[CR]  | Transmits a FD extended data frame without bit rate switch.
'b' | B  |  YES  |  biiildd...[CR]       | Transmits a FD base data frame with bit rate switch.
'B' | B  |  YES  |  Biiiiiiiildd...[CR]  | Transmits a FD extended data frame with bit rate switch.
'P' | A  |   -   |  P[CR]                | Polls incoming FIFO for CAN frames (single poll).
'A' | A  |   -   |  A[CR]                | Polls incoming FIFO for CAN frames (all pending frames).
'F' | A  |  YES  |  F[CR]                | Reads status flags.
'f' | Y  |  YES  |  f[CR]                | Reads detailed status.
'X' | A  |   -   |  Xn[CR]               | Sets Auto Poll/Send ON/OFF for received frames.
    | CB |  YES  |                       | Sets the device to firmware update mode
'W' | A  |  YES  |  Wn[CR]               | Sets up filter mode setting.
    |    |       |                       | W0 Dual filter mode (N/A)
    |    |       |                       | W1 Single filter mode (N/A)
    | Y  |       |                       | W2 Simple filter mode
'M' | A  |  YES  |  Mxxxxxxxx[CR]        | Sets Acceptance Code Register (ACn Register).
'm' | A  |  YES  |  mxxxxxxxx[CR]        | Sets Acceptance Mask Register (AMn Register).
'U' | A  |   -   |  Un[CR]               | Sets up UART with a new baud rate where n is 0-6.
'V' | A  |  YES  |  V[CR]                | Gets software and hardware version characters.
'v' | Y  |  YES  |  v[CR]                | Gets detailed version information.
'I' | YD |  YES  |  I[CR]                | Gets CAN FD controller information.
'i' | YD |  YES  |  i[CR]                | Gets detailed CAN FD controller information.
'N' | A  |  YES  |  N[CR]                | Gets serial number of the hardware.
    | Y  |       |  Nxxxx[CR]            | Sets serial number of the hardware to xxxx,
    |    |       |                       | where the new serial number xxxx is a hex value.
'Z' | A  |  YES  |  Zn[CR]               | Sets timestamp ON/OFF for received frames only.
    |    |       |                       | Z0 No timestamp
    |    |       |                       | Z1 Millisecond timestamp
    | Y  |       |                       | Z2 Microsecond timestamp
    | Z  |       |  Z[CR]                | Gets current timestamp
'z' | Z  |  YES  |  znxyy[CR]            | Sets the reporting mechanism,
    |    |       |                       | where x and yy are hex values.
    | Z  |       |  z[CR]                | Gets detailed time
'Q' | A  |  YES  |  Qn[CR]               | Sets auto startup feature ON/OFF (from power on). 
    |    |       |                       | Q0 Auto startup off
...
- `Y` : some forked project of CANable 2.0 project
- `Z` : some forked project of USB2CANFDV1 project
- `CB`: LAWICEL AB then replaced by [USB2CANFDV1](https://github.com/WeActStudio/WeActStudio.USB2CANFDV1)  
        The replacement was mentioned in CANable 2.0 project in a [comment](https://github.com/normaldotcom/canable2-fw/blob/b158aa7ab106bc018715cd4d5f02f544f9de1830/src/slcan.c#L289)
- `YA`: LAWICEL AB then replaced by some forked project of CANable 2.0 project,  
        following a similar command deployed by USB2CANFDV1
- `YC`: some forked project of CANable 2.0 project following a similar command deployed by USB2CANFDV1 project
- `YD`: some forked project of CANable 2.0 project following a similar command proposed on [mikrocontroller.net](https://www.mikrocontroller.net/topic/501223)
